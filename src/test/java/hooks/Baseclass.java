...
public class Baseclass {
	  protected WebDriver driver;
	    ...
	    public Baseclass() {
	        // Initialize driver and test for THIS thread/scenario
	        this.driver = Driver.getDriver();  // ThreadLocal safe
	        this.test = Hooks.getExtentTest(); // ThreadLocal safe
...
//	FluentWait<WebDriver> wait = new FluentWait<>(driver).withTimeout(Duration.ofSeconds(60)) // Max wait time
...
//	ExtentTest test = Hooks.getExtentTest();
	...
	private void logFontValidation(boolean isPassed, String Expected, String Actual, ExtentTest test, WebElement web) {
		...
		// "<br><img src='data:image/png;base64," + screenshotBase64 + "' height='200'
		// width='300'/>";
		// Log pass/fail status with embedded image
		if (isPassed) {
			test.pass(logMessage).info("Expected result: " + Expected).info("Actual result:" + Actual);
		} else {
			test.fail(logMessage).info("Expected result: " + Expected).info("Actual result:" + Actual);
		}
	}
	...
	private void logStatus(boolean isPassed, String Expected, String Actual, ExtentTest test, WebElement web) {
		...
		if (isPassed) {
			test.pass(logMessage).info("Actual result: " + Expected).info("Expected result: " + Actual);
		} else {
			test.fail(logMessage).info("Actual result: " + Expected).info("Expected result: " + Actual);
		}
	}
	...
	public void validateattribute(WebElement element, String attribute, String expectedValue) {
		try {
			...
			logStatus(isMatched, "Attribute " + attribute + ": " + element.getAttribute(attribute), expectedValue, test,
					element);
			validatefont(element);
	...
	public void dispalyedattribute(WebElement element, String message) {
		try {
			...
			System.out
					.println(isDisplayed ? element.getText() + "is displayed" : element.getText() + "is not displayed");
			logStatus(isDisplayed, message + " is " + (isDisplayed ? "displayed" : "not displayed"), message, test,
					element);
//	        validatefont(element);
...
//    }
	...
	public void validatealert(WebElement element, String expectedText, String wrongexpect) {
		try {
			boolean isMatched = element.isDisplayed()
					&& (element.getText().contains(expectedText) || element.getText().contains(expectedText));
			System.out.println(isMatched ? element.getText() + " is matched" : element.getText() + " is not matched");
		...
		for (int i = 0; i < tableElements.size(); i++) {
			if (tableElements.get(i).isDisplayed()) {
				try {
					...
					System.err.println(
							"Stale Element: The element is no longer attached to the DOM - " + tableElements.get(i));
				} catch (ElementClickInterceptedException e) {
					System.err.println("Element Click Intercepted: Another element is blocking the click - "
							+ tableElements.get(i));
				} catch (Exception e) {
					System.err.println("Exception occurred while clicking the element - " + tableElements.get(i) + " : "
							+ e.getMessage());
				}
			}
		}
...
//		wait.until(ExpectedConditions.visibilityOf(alert_popup));
	...
	public void Filtersendkeysmultipleweb(List<WebElement> elements, String text) {
		elements.stream().filter(WebElement::isDisplayed).findFirst() // Stops processing the stream once the first
																		// match is found
